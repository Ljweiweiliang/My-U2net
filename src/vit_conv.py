...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = x1[0].permute(0, 2, 1)  #[1, 64, 20736]
        #print("qkv:", x1.size())
        sqrt_size = int(np.sqrt(x1.shape[2]))
        #x1 = x1.view(x1.shape[0], x1.shape[1], np.sqrt(x1.shape[2]).astype(int), np.sqrt(x1.shape[2]).astype(int))
        x1 = x1.view(x1.shape[0], x1.shape[1], sqrt_size, sqrt_size)
        #print("qkv:view", x1.size())
        x1 = F.dropout(x1, self.proj_drop)
        #print("qkv:", x1.size())
        #print("qkv:x1", x1.size())
        return x1
...
class Transformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.attention_output = Attention(channels=out_channels, num_heads=num_heads,proj_drop=proj_drop,
                                          padding_q=padding_q, padding_kv=padding_kv,stride_kv=stride_kv,
                                          stride_q=stride_q,attention_bias=attention_bias,)
...
class Block_encoder_bottleneck(nn.Module):
    def  __init__(self, blk, in_channels, out_channels, att_heads, dpr):
        ...
        if ((self.blk=="first") or (self.blk=="bottleneck")):
            ...
            self.conv1 = nn.Conv2d(in_channels, out_channels, 3, 1, padding="same") #32 64
            self.conv2 = nn.Conv2d(out_channels, out_channels, 3, 1, padding="same")#64 64
    ...
    def forward(self, x):
        if ((self.blk=="first") or (self.blk=="bottleneck")):
            #print(x.size())
            x1 = x.permute(0, 2, 3, 1)  #[1, 288, 288, 32]
            #print("x1:",x1.size())
            x1 = self.layernorm(x1)#[1, 288, 288, 32]
            #print(x1.size())
            x1 = x1.permute(0, 3, 1, 2)#[1, 32, 288, 288]
            #print(x1.size())
            x1 = F.relu(self.conv1(x1))#[1, 64, 288, 288]
            #print(x1.size())
            x1 = F.relu(self.conv2(x1))#[1, 64, 288, 288]
            #print(x1.size())
            x1 = F.dropout(x1, 0.3)
            x1 = F.max_pool2d(x1, (2,2)) #[1, 64, 144, 144]
            #print(x1.size())
            out = self.trans(x1)
            #print(out.size())
            # without skip
