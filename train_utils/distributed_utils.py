...
class F1Score(object):
    ...
    def update(self, pred: torch.Tensor, gt: torch.Tensor):
        ...
        assert batch_size == 1, f"validation mode batch_size must be 1, but got batch_size: {batch_size}."
        resize_pred = F.interpolate(pred, (h, w), mode="bilinear", align_corners=False)
        gt_num = torch.sum(torch.gt(gt, self.threshold).float())   #TP+FN  实际正例

        pp = resize_pred[torch.gt(gt, self.threshold)]  # 对应预测map中GT为前景的区域  TP
        nn = resize_pred[torch.le(gt, self.threshold)]  # 对应预测map中GT为背景的区域  FP
        ...
        precision = pp_hist_flip_cum / (pp_hist_flip_cum + nn_hist_flip_cum + 1e-4)  #TP/(TP+FP)
        recall = pp_hist_flip_cum / (gt_num + 1e-4)  #TP/(TP+FN)
...
class F1Score_pre(object):
    ...
    def compute(self):
        pre_mean = self.precision_cum / self.num_cum
        rec_mean = self.recall_cum / self.num_cum
        pre_mean = torch.amax(pre_mean).item()
        return pre_mean
    ...
    def __str__(self):
        pre_mean = self.compute()
        return f'pre: {pre_mean:.3f}'


class F1Score_rec(object):
    """
    refer: https://github.com/xuebinqin/DIS/blob/main/IS-Net/basics.py
    """
    ...
    def __str__(self):
        ...
        return f'acc: {rec_mean:.3f}'


class F1Score_acc_1(object):
    """
    refer: https://github.com/xuebinqin/DIS/blob/main/IS-Net/basics.py
    """
    ...
    def update(self, pred: torch.Tensor, gt: torch.Tensor):
        ...
        resize_pred = F.interpolate(pred, (h, w), mode="bilinear", align_corners=False)
        gt_num = torch.sum(torch.gt(gt, self.threshold).float())  # TP+FN  实际正例
        pred_num = torch.sum(torch.le(pred,self.threshold).float())  #FP+TN
        pp = resize_pred[torch.gt(gt, self.threshold)]  # 对应预测map中GT为前景的区域  TP
        nn = resize_pred[torch.le(gt, self.threshold)]  # 对应预测map中GT为背景的区域  FP
        aa = resize_pred[torch.le(resize_pred,self.threshold)]  # 对应预测map中GT为背景的区域  TN   (1,1,320,320)

        pp_hist = torch.histc(pp, bins=255, min=0.0, max=1.0)
        nn_hist = torch.histc(nn, bins=255, min=0.0, max=1.0)
        aa_hist = torch.histc(aa, bins=255, min=0.0, max=1.0)

        # Sort according to the prediction probability from large to small
        pp_hist_flip = torch.flipud(pp_hist)
        nn_hist_flip = torch.flipud(nn_hist)
        aa_hist_flip = torch.flipud(aa_hist)

        pp_hist_flip_cum = torch.cumsum(pp_hist_flip, dim=0)
        nn_hist_flip_cum = torch.cumsum(nn_hist_flip, dim=0)
        aa_hist_flip_cum = torch.cumsum(aa_hist_flip, dim=0)

        precision = pp_hist_flip_cum / (pp_hist_flip_cum + nn_hist_flip_cum + 1e-4)  # TP/(TP+FP)
        recall = pp_hist_flip_cum / (gt_num + 1e-4)  # TP/(TP+FN)
        accuracy_1 =(pp_hist_flip_cum+aa_hist_flip_cum)/(gt_num+pred_num+1e-4)  #TP+TN/(TP+FP+FN+TN)
        ...
        if self.accuracy_1_cum is None:
            self.accuracy_1_cum = torch.full_like(accuracy_1, fill_value=0.)
        ...
        self.precision_cum += precision
        self.recall_cum += recall
        self.accuracy_1_cum += accuracy_1
        self.num_cum += batch_size

    def compute(self):
        pre_mean = self.precision_cum / self.num_cum
        rec_mean = self.recall_cum / self.num_cum
        acc_mean = self.accuracy_1_cum / self.num_cum

        acc = torch.amax(acc_mean).item()
        return acc
    ...
    def reduce_from_all_processes(self):
        ...
        torch.distributed.all_reduce(self.precision_cum)
        torch.distributed.all_reduce(self.recall_cum)
        torch.distributed.all_reduce(self.accuracy_1_cum)
        torch.distributed.all_reduce(self.num_cum)

    def __str__(self):
        acc_1 = self.compute()
        return f'acc_1: {acc_1:.3f}'


class F1Score_acc_2(object):
    """
    refer: https://github.com/xuebinqin/DIS/blob/main/IS-Net/basics.py
    """
    ...
    def update(self, pred: torch.Tensor, gt: torch.Tensor):
        ...
        pp = resize_pred[torch.gt(gt, self.threshold)]  # 对应预测map中GT为前景的区域  TP
        nn = resize_pred[torch.le(gt, self.threshold)]  # 对应预测map中GT为背景的区域  FP
        aa = resize_pred[torch.le(resize_pred, self.threshold)]  # 对应预测map中GT为背景的区域  TN
        bb = resize_pred[torch.gt(resize_pred, self.threshold)]# 对应预测map中GT为背景的区域  FN

        pp_hist = torch.histc(pp, bins=255, min=0.0, max=1.0)
        nn_hist = torch.histc(nn, bins=255, min=0.0, max=1.0)
        aa_hist = torch.histc(aa, bins=255, min=0.0, max=1.0)
        bb_hist = torch.histc(bb, bins=255, min=0.0, max=1.0)
        # Sort according to the prediction probability from large to small
        pp_hist_flip = torch.flipud(pp_hist)
        nn_hist_flip = torch.flipud(nn_hist)
        aa_hist_flip = torch.flipud(aa_hist)
        bb_hist_flip = torch.flipud(bb_hist)
        pp_hist_flip_cum = torch.cumsum(pp_hist_flip, dim=0)
        nn_hist_flip_cum = torch.cumsum(nn_hist_flip, dim=0)
        aa_hist_flip_cum = torch.cumsum(aa_hist_flip, dim=0)
        bb_hist_flip_cum = torch.cumsum(bb_hist_flip, dim=0)
        precision = pp_hist_flip_cum / (pp_hist_flip_cum + nn_hist_flip_cum + 1e-4)  # TP/(TP+FP)
        recall = pp_hist_flip_cum / (gt_num + 1e-4)  # TP/(TP+FN)
        accuracy_2 = (pp_hist_flip_cum + aa_hist_flip_cum) / (pp_hist_flip_cum+ nn_hist_flip_cum+aa_hist_flip_cum+bb_hist_flip_cum +1e-4)  # TP+TN/(TP+FP+FN+TN)
        ...
        if self.accuracy_2_cum is None:
            self.accuracy_2_cum = torch.full_like(accuracy_2, fill_value=0.)
        ...
        self.precision_cum += precision
        self.recall_cum += recall
        self.accuracy_2_cum += accuracy_2
        self.num_cum += batch_size

    def compute(self):
        pre_mean = self.precision_cum / self.num_cum
        rec_mean = self.recall_cum / self.num_cum
        acc_mean = self.accuracy_2_cum / self.num_cum

        acc_2 = torch.amax(acc_mean).item()
        return acc_2
    ...
    def reduce_from_all_processes(self):
        ...
        torch.distributed.all_reduce(self.precision_cum)
        torch.distributed.all_reduce(self.recall_cum)
        torch.distributed.all_reduce(self.accuracy_2_cum)
        torch.distributed.all_reduce(self.num_cum)

    def __str__(self):
        acc_2 = self.compute()
        return f'acc_2: {acc_2:.3f}'
